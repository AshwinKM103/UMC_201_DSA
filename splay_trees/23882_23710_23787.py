...
def get_nearest_neighbors(E, estimate_vector):
    # using heapify approach because it is faster than sorting and any other appraoch we tried 
    # the approaches we tries were Quickselect, median of medians, and heapify of which heapify was the most efficient
    # building a min heap and returning the first 6 digit's index 
    distance_map = [((E - float(val)) ** 2, idx) for idx, val in enumerate(estimate_vector)]
    """Apply heap to get the closest 6 neighbors"""
    heapq.heapify(distance_map)
    return [idx for dist, idx in heapq.nsmallest(6, distance_map)]    
